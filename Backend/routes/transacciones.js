...
router.put('/:id', verificarToken, upload.single('imagen'), manejarErrorSubida, async (req, res) => {
    const { id } = req.params;
    const { concepto, monto, tipo, categoria } = req.body;

    try {
        const existente = await pool.query('SELECT usuario_id FROM transacciones WHERE id = $1', [id]);
        if (existente.rows.length === 0) {
            return res.status(404).json({ error: "No se encontró la transacción" });
        }
        if (!esPropietarioOAdmin(existente.rows[0], req)) {
            return res.status(403).json({ error: "No tienes permiso para editar esta transacción" });
        }
...
router.get('/:id/recibo', verificarToken, async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query('SELECT usuario_id, imagen_path FROM transacciones WHERE id = $1', [id]);
